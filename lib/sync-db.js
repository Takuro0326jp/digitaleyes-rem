...
function remoteDatabaseUrlFromEnv() {
  return (
...
/** ローカル（VERCEL なし）で .env の libsql を使うときのみ true。未設定ならローカル file DB を使い本番と混線しない */
function useRemoteSyncDbOnLocal() {
  const v = String(process.env.REM_USE_REMOTE_SYNC_DB || "").trim().toLowerCase();
  return v === "1" || v === "true" || v === "yes";
}

function localFileSyncDbUrl() {
...
function resolveDatabaseUrl() {
  const remote = remoteDatabaseUrlFromEnv();

  // Vercel 本番・Preview: 環境変数のリモート URL をそのまま使用（未設定時のみメモリ）
  if (process.env.VERCEL) {
    if (remote) return remote;
    return "file::memory:?cache=shared";
  }

  // ローカル proxy / node: 既定は data/sync.db（.env に本番 DATABASE_URL があっても無視し、誤って本番を触らない）
  if (remote && useRemoteSyncDbOnLocal()) return remote;
  return localFileSyncDbUrl();
}
...
async function getDb() {
  ...
  const authToken =
    ...
    undefined;
  if (String(url).startsWith("libsql://") && !authToken) {
    throw new Error(
      "DATABASE_URL が libsql://（Turso）のときは TURSO_AUTH_TOKEN または TURSO_API_TOKEN が必須です。Vercel の Environment Variables に設定して Redeploy してください。"
    );
  }
...
async function migrate(c) {
  const ddl = [
    ...
    `CREATE TABLE IF NOT EXISTS customer_status_daily (
      property_id TEXT NOT NULL,
      customer_id TEXT NOT NULL,
      status_date TEXT NOT NULL,      -- YYYY-MM-DD
      status TEXT NOT NULL,
      changed_at TEXT,                -- YYYY-MM-DD HH:MM:SS
      source TEXT,                    -- sync / manual
      PRIMARY KEY (property_id, customer_id, status_date)
    )`,
    `CREATE INDEX IF NOT EXISTS idx_csd_prop_date_status ON customer_status_daily(property_id, status_date, status)`,
    `CREATE INDEX IF NOT EXISTS idx_csd_prop_cust_date ON customer_status_daily(property_id, customer_id, status_date)`,
  ...
  if (!(await tableHasColumn(c, "customer_snapshot", "tel"))) {
    await c.execute(
      "ALTER TABLE customer_snapshot ADD COLUMN tel TEXT"
    );
  }
  await c.execute(
    `CREATE INDEX IF NOT EXISTS idx_cs_prop_tel ON customer_snapshot(property_id, tel)`
  );
  await c.execute(
  ...
  await c.execute(
    `UPDATE customer_snapshot
     SET tel = COALESCE(NULLIF(tel, ''), json_extract(payload, '$."c.tel"'))
     WHERE tel IS NULL OR tel = ''`
  );
  await c.execute(
    `INSERT OR IGNORE INTO customer_status_daily(property_id, customer_id, status_date, status, changed_at, source)
     SELECT
       property_id,
       customer_id,
       COALESCE(NULLIF(substr(replace(replace(upd_date, '/', '-'), 'T', ' '), 1, 10), ''), date('now')) AS status_date,
       trim(status) AS status,
       COALESCE(NULLIF(replace(replace(upd_date, '/', '-'), 'T', ' '), ''), datetime('now')) AS changed_at,
       'bootstrap'
