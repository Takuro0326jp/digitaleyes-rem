...
async function createProperty(userId, payload) {
  ...
  const extraInit = {};
  const cn = String(payload.clientName || "").trim();
  if (cn) extraInit.clientName = cn;
  const extraJson = JSON.stringify(extraInit);
  await db.execute({
    sql: `INSERT INTO property_master
          (id, user_id, name, database_id, database_password, table_name, extra_json, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`,
    args: [id, userId, payload.name, payload.databaseId, payload.databasePassword || "", payload.tableName, extraJson],
  });
  return getPropertyByIdForUser(id, userId);
}

/**
 * クライアント（企業名）と物件の紐づけを同期。propertyIds に含まれる物件は clientName を設定、
 * 含まれず現状が同一企業名の物件は紐づけ解除（空文字）。
 */
async function syncPropertiesClientName(tenantId, clientName, linkedPropertyIds, actorName = "") {
  await migratePropertiesTable();
  const target = String(clientName || "").trim();
  if (!target) throw new Error("クライアント名が空です");
  const want = new Set((linkedPropertyIds || []).map(String));
  const all = await listPropertiesByUser(tenantId);
  for (const p of all) {
    const id = String(p.id);
    const cur = String(p.clientName || "").trim();
    if (want.has(id)) {
      if (cur !== target) {
        await updatePropertyMeta(id, tenantId, { clientName: target }, actorName);
      }
    } else if (cur === target) {
      await updatePropertyMeta(id, tenantId, { clientName: "" }, actorName);
    }
  }
  return { ok: true };
}
...
async function addPropertyImage(propertyId, userId, payload, actorName = "") {
  await migratePropertiesTable();
  ...
  await db.execute({
    ...
    args: [
      ...
      userId,
      actorName || "",
      JSON.stringify({ type: "property_image_added", imageId: id, filename, mimeType }),
    ],
  });
  return { id };
}

async function deletePropertyImage(propertyId, userId, imageId, actorName = "") {
  await migratePropertiesTable();
  const db = await getDb();
  const before = await db.execute({
    sql: "SELECT filename, mime_type FROM property_images WHERE id = ? AND property_id = ? AND user_id = ? LIMIT 1",
