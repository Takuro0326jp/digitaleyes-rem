...
function isNagaokakyoRow(state, city) {
  const st = String(state || "");
  const ct = String(city || "");
  return st.includes("兵庫") && (ct.includes("長岡京") || ct.includes("長岡京市"));
}
...
async function buildCustomerAnalysisReport({ propertyId, property, weekStart, weekEnd }) {
  ...
  const r = await db.execute({
    sql: `SELECT customer_id, date_entry, upd_date, status, state, city, baitai, yosan, ninzu, jikosikin,
                 questionnaire23, questionnaire24, payload
  ...
  for (const raw of rows) {
    ...
    const pref = labelOrMissing(pickField(p, denorm, ["都道府県"], ["c.state"]));
    const stateVal = String(pickField(p, denorm, ["都道府県"], ["c.state"]) || denorm.state || "").trim();
    ...
    const town = labelOrMissing(pickField(p, denorm, ["町村"], ["c.town"]));
    if (isNagaokakyoRow(stateVal || pref, cityValRaw)) {
      const areaLabel = town !== "未記入" ? town : labelOrMissing(cityValRaw);
      if (!areaMap.has(areaLabel)) areaMap.set(areaLabel, bucket3());
      const ab = areaMap.get(areaLabel);
