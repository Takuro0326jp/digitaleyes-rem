...
const { ROLE, isMaster } = require("./roles");
...
async function migrateUsers() {
  ...
  try {
    await db.execute("ALTER TABLE users ADD COLUMN password_reset_expires_ms INTEGER");
  } catch (_) {}
  try {
    await db.execute(
...
async function maybeBootstrapMaster() {
  ...
  const r = await db.execute({
    sql: "SELECT id, email, password, name, active_property_id, login_default_property_id, role, tenant_id, client, property_ids_json, must_change_password FROM users WHERE lower(trim(email)) = ?",
    args: [email],
  });
...
function rowToUser(row, columnNames) {
  ...
  return {
    ...
    propertyIds,
    mustChangePassword:
      Number(r.must_change_password ?? r.mustChangePassword ?? 0) === 1,
  };
}
...
async function listUsers() {
  ...
  const db = await getDb();
  const r = await db.execute(
    "SELECT id, email, password, name, active_property_id, login_default_property_id, role, tenant_id, client, property_ids_json, must_change_password FROM users ORDER BY id"
  );
  const cols = normalizeUserListColumnNames(r.columns || []);
...
async function insertUser(u) {
  await migrateUsers();
  const db = await getDb();
  const mustCh = u.mustChangePassword ? 1 : 0;
  await db.execute({
    sql: `INSERT INTO users (id, email, password, name, active_property_id, login_default_property_id, role, tenant_id, client, property_ids_json, must_change_password)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [
      String(u.id),
      String(u.email || "").toLowerCase().trim(),
...
async function updateUser(u) {
  ...
  const mustCh = u.mustChangePassword ? 1 : 0;
  await db.execute({
    sql: `UPDATE users SET email=?, password=?, name=?, active_property_id=?, login_default_property_id=?, role=?, tenant_id=?, client=?, property_ids_json=?, must_change_password=?
          WHERE id=?`,
    args: [
      String(u.email || "").toLowerCase().trim(),
      String(u.password || ""),
...
async function deleteUserById(id) {
  ...
  if (u && isMaster(u.role)) {
    throw new Error("マスター権限のユーザーは削除できません");
  }
  const db = await getDb();
  await db.execute({ sql: "DELETE FROM users WHERE id = ?", args: [String(id)] });
}

async function setPasswordResetTokenByEmail(email, token, expiresMs) {
  await migrateUsers();
  const db = await getDb();
  const em = String(email || "").toLowerCase().trim();
  await db.execute({
    sql: `UPDATE users SET password_reset_token = ?, password_reset_expires_ms = ? WHERE lower(trim(email)) = ?`,
    args: [String(token || ""), Number(expiresMs) || 0, em],
  });
}

async function clearPasswordResetByEmail(email) {
  await migrateUsers();
  const db = await getDb();
  const em = String(email || "").toLowerCase().trim();
  await db.execute({
    sql: `UPDATE users SET password_reset_token = NULL, password_reset_expires_ms = NULL WHERE lower(trim(email)) = ?`,
    args: [em],
  });
}
 ...
 * @param {string} newPasswordHash sha256 hex
 * @returns {{ ok: true, user: object } | { ok: false, message: string }}
 */
async function consumePasswordReset(tokenPlain, newPasswordHash) {
  await migrateUsers();
  const t = String(tokenPlain || "").trim();
  if (!t) return { ok: false, message: "再設定用のリンクが無効です。" };
  const db = await getDb();
  const r = await db.execute({
    sql: `SELECT id, email, password, name, active_property_id, login_default_property_id, role, tenant_id, client, property_ids_json, password_reset_expires_ms, must_change_password FROM users WHERE password_reset_token = ?`,
    args: [t],
  });
  const cols = normalizeUserListColumnNames(r.columns || []);
  const rows = r.rows || [];
  if (!rows.length) return { ok: false, message: "再設定用のリンクが無効か、すでに使用されています。" };
  const row = rows[0];
  const o = rowToObject(row, cols);
  const exp = Number(o.password_reset_expires_ms ?? 0);
  if (!exp || Date.now() > exp) {
    return { ok: false, message: "リンクの有効期限が切れています。再度「パスワードを忘れた方はこちら」から手続きしてください。" };
  }
  const user = rowToUser(row, cols);
  await db.execute({
    sql: `UPDATE users SET password = ?, password_reset_token = NULL, password_reset_expires_ms = NULL, must_change_password = 0 WHERE id = ?`,
    args: [String(newPasswordHash), String(user.id)],
  });
  return { ok: true, user: { ...user, password: String(newPasswordHash) } };
}
