...
async function consumePasswordReset(tokenPlain, newPasswordHash) {
  ...
  const db = await getDb();
  const r = await db.execute({
    sql: `SELECT id, email, password, name, active_property_id, login_default_property_id, role, tenant_id, client, property_ids_json, password_reset_expires_ms FROM users WHERE password_reset_token = ?`,
    args: [t],
  });
  const cols = normalizeUserListColumnNames(r.columns || []);
  const rows = r.rows || [];
  ...
  await db.execute({
    sql: `UPDATE users SET password = ?, password_reset_token = NULL, password_reset_expires_ms = NULL WHERE id = ?`,
