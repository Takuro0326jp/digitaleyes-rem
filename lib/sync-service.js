...
async function queryLocalCustomers(propertyId, query) {
  ...
  const dataR = await db.execute({
    sql: `SELECT customer_id, mail, upd_date, payload, status FROM customer_snapshot WHERE ${whereSql} ORDER BY upd_date DESC`,
    args,
  });
  const rows = (dataR.rows || []).map((row) => {
    const payloadRaw = row.payload ?? row[3] ?? row[0];
    let payload = typeof payloadRaw === "string" ? JSON.parse(payloadRaw) : payloadRaw;
    payload = payload && typeof payload === "object" ? { ...payload } : {};
    const statusCol = row.status ?? row[4];
    const st = statusCol != null && String(statusCol).trim() !== "" ? String(statusCol).trim() : "";
    const stPayload = String(payload["c.status"] ?? "").trim();
    if (st && !stPayload) payload["c.status"] = st;
    return {
