...
function mapPutObjectAccessError(err, bucketName) {
  const name = String(err && err.name || "");
  const msg = String(err && err.message || "");
  const denied =
    name === "AccessDenied" ||
    /not authorized/i.test(msg) && /s3:PutObject/i.test(msg);
  if (!denied) return null;
  const b = bucketName ? `「${bucketName}」` : "対象バケット";
  return new Error(
    `S3へのアップロードが拒否されました（${b} への PutObject 権限がありません）。` +
      "サーバー環境変数の AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY が、メール送信用（SES SMTP ユーザーなど）だけの権限になっていないか確認し、" +
      "そのバケットに s3:PutObject（必要なら GetObject 等）が付いた IAM ユーザーのキーを設定してください。"
  );
}
...
async function uploadObjectDirect({ bucketName, key, mimeType, body, region }, env = process.env) {
  ...
  try {
    ...
    const mapped = mapPutObjectAccessError(e, bucketName);
