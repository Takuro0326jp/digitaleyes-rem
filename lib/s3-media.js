...
async function createUploadPresignedUrl({ bucketName, key, mimeType, fileSize, userId, accountId }, env = process.env) {
  const e = envOf(env);
...
async function createReadSignedUrl({ bucketName, key, expiresIn = 3600 }, env = process.env) {
...
async function removeObject({ bucketName, key }, env = process.env) {
  if (!bucketName || !key) return;
  const s3 = s3Client(env);
